...
def ASR_predict_route():
    ...
    if file:
        ...
        if file_type=="mp3" or file_type=="m4a" or file_type=="wav":
            ...
            if db_api_key:
                api_key,key_api_key=db_api_key.key,db_api_key.aes_key
            if db_access_token:
                access_token,key_access_token=db_access_token.token,db_access_token.aes_key
            ASR_predict.delay(id=file_id, file=filepath, prompt=prompt, api_key=api_key, access_token=access_token, key_api_key=key_api_key, key_access_token=key_access_token)

            new_file=File(id=file_id, user_mail=user.mail, status="PENDING", type="ASR", resource=filepath,)
...
def OCR_predict_route():
    ...
    if file:
        ...
        if file_type=="jpg" or file_type=="jpeg" or file_type=="png":
            ...
            if db_access_token:
                access_token, key_access_token = db_access_token.token, db_access_token.aes_key
            OCR_predict.delay(id=file_id, file=filepath, prompt=prompt, api_key=api_key, access_token=access_token, key_api_key=key_api_key, key_access_token=key_access_token)


            new_file=File(id=file_id, user_mail=user.mail, status="PENDING", type="OCR", resource=filepath)
...
def OCR_predict_Text_route(args):
    ...
    if db_access_token:
        access_token, key_access_token = db_access_token.token, db_access_token.aes_key
    file_id = str(uuid.uuid4())
    OCR_predict_Text.delay(id=file_id, content=content, prompt=prompt, api_key=api_key, access_token=access_token, key_api_key=key_api_key, key_access_token=key_access_token)

    new_file = File(id=file_id, user_mail=user.mail, status="PENDING", type="OCR", resource=PURE_TEXT)
    db.session.add(new_file)
    db.session.commit()
    return "", 200
