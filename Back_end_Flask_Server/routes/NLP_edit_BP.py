...
def ASR_NLP_edit(args,file_id):
    ...
    if file:
        ...
        if file.user_mail==current_user:
            ...
            NLP_edit_ASR.delay(id=file_id, prompt=prompt, content=content, api_key=api_key, access_token=access_token, key_api_key=key_api_key, key_access_token=key_access_token)
            file.status="PENDING"
            db.session.commit()
        else:
            return "",403
...
def OCR_NLP_edit(args,file_id):
    ...
    if file:
        ...
        if file.user_mail == current_user:
            ...
            if db_access_token:
                access_token, key_access_token = db_access_token.token, db_access_token.aes_key
            NLP_edit_OCR.delay(id=file_id, prompt=prompt, content=content, api_key=api_key, access_token=access_token, key_api_key=key_api_key, key_access_token=key_access_token)
            file.status = "PENDING"
            db.session.commit()
        else:
            return "", 403
