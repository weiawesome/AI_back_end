...
def get_default_prompt(mode):
    ...
    if(mode=='OCR'):
        default_OCR_prompt = """你現在是學測英文作文的閱卷委員，請依照下面的rubric，評分考生在內容、組織、文法句構、字彙拼字之表現，各項得分加總後給予一個整體分數(holistic score)，再依總分1至20分，分為下述五等級:特優(19-20分)、優(15-18分)、可 (10-14分)、差(5-9分)、劣(0-4分)。你在仔細評閱考生的作答內容後，再依其內容是否切題、組織是否具連貫性、句子結構與文法、用字是否適切表達文意，及拼字與標點符號使用是否正確等要項，進行評分。並且，字數不足120字者，扣總分1分;未分段者，亦扣總分1分。
rubric:
    1.內容
