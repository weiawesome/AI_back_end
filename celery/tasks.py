...
from utils import strid2byte,decrypt
...
def ASR_predict(self,id,file, prompt,api_key,access_token,key_api_key,key_access_token):
    ...
    if revChat_test(access_token=access_token):
        ...
        details, result = openaiChat(api_key=api_key, mode="ASR", prompt=prompt, text=content)
        response["result"]=result
        response["details"]=details
    return response
...
def OCR_predict(self,id,file, prompt,api_key,access_token,key_api_key,key_access_token):
    ...
    if revChat_test(access_token=access_token):
        ...
        details, result = openaiChat(api_key=api_key, mode="OCR", prompt=prompt, text=content)
        response["result"] = result
        response["details"] = details
    return response
